...
public:
    ...
    void fit(const std::vector<std::vector<double>>& trainData, const std::vector<double>& trainLabels, double learningRate, int iterations);
    std::vector<double> predict(const std::vector<std::vector<double>>& testData);
    double predict(const std::vector<double>& singleDataPoint);
