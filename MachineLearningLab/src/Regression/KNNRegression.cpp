...
std::vector<double> KNNRegression::predict(const std::vector<std::vector<double>>& X_test) const {
	...
	*/
	...
	for (int i = 0; i < X_test.size(); i++) {
		...
		// Get the label's value for the nth nearest item and add it to the sum
		for (int j = 0; j < k_; j++) {
			std::pair<int, double> nth_nearest = distances[j];
			double label = y_train_[nth_nearest.first];
			sum += label;
		}
