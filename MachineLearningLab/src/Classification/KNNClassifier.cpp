...
std::vector<double> KNNClassifier::predict(const std::vector<std::vector<double>>& X_test) const {
	...
	*/
	...
	for (int i = 0; i < X_test.size(); i++) {
		...
		std::sort(distances.begin(), distances.end(), [](const std::pair<int, double>& left, const std::pair<int, double>& right) {
			return left.second < right.second;
			});


		// Count the occurunces of each label
		std::map<double, double> label_count;

		for (int j = 0; j < k_; j++) {
			std::pair<int, double> n_nearest = distances[j];
			double label = y_train_[n_nearest.first];
			label_count[label]++;
		}

		// Find the most repeated label
		double selected_label = -1;
		double max_value = -1;

		for (auto& pair : label_count) {
			if (pair.second > max_value) {
				max_value = pair.second;
				selected_label = pair.first;
			}
		}

		y_pred.push_back(selected_label);
	}
